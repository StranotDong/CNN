...
class basic_model:
	...
	def _conv_layer(self, inputs, i, is_training, init=None, reg=None):
		...
		with tf.variable_scope('conv_layer'+str(i)) as scope:
			# conv layer
	...
	def _hidden_layer(self, inputs, i, is_training, init=None, reg=None):
		...
		with tf.variable_scope('Hidden'+str(i)) as scope1:
			## Linear
...
"""
This model is used for transfer learning from basic model
"""

class transfer_learning_model(basic_model):
	'''
	The saver used to restore the reuse var
	'''
	def reuse_var_saver(self, scope):
	...
	# '''
	# The unfrozen vars for training
	# '''
	# def train_vars(self, scope):
	# 	vars_ = tf.get_collection(
	# 		tf.GraphKeys.TRAINABLE_VARIABLES,
	# 		scope=scope)
	...
	Inputs:
		data_size: [data_length, data_width, data_depth]
		train_var_scope: unfrozen training vars
	...
	Returns:
		...
		learning_rate: current learning rate
	'''
	def model(self, data_size, train_var_scope):
		# 1. Deciding the placeholders
		## input:
		data_length, data_width, data_depth = data_size # input size
		X = tf.placeholder(tf.float32, 
		...
		## set the unfrozen training vars
		train_vars = tf.get_collection(
			tf.GraphKeys.TRAINABLE_VARIABLES,
			scope=train_var_scope)
	...
	This function is used to train. The check points will be stored in ckpt folder if needed. It will
	do the validation after training if val_data and val_labels are not None.
	The reuse part of the original model and new model should be the same.
	Inputs:
		data: total training data: shape is [number, length, width, depth]
		labels: total training labels
		val_data: validation data
		val_labels: validation labels
		original_model_path: the path of original model
		reuse_var_scope: reuse the vars in the scope from original model
		train_var_scope: unfrozen train vars
		last_frozen layer: the last frozen layer
	...
	def train(
			self, 
			original_model_path, reuse_var_scope,
			train_var_scope,
			# last_frozen_layer,
			data, labels, 
			val_data=None, val_labels=None,
		...
		# construct the model
		data_size = data.shape[1:]
		placeholder_dict, updated_node_dict = self.model(data_size, train_var_scope)
		...
		# saver to restore the original model
		original_saver = self.reuse_var_saver(reuse_var_scope)
		...
		with tf.Session() as sess:
			# initialize
			sess.run(tf.global_variables_initializer())
			original_saver.restore(sess, original_model_path)
			# freeze_out = sess.run(
			# 	last_frozen_layer,
